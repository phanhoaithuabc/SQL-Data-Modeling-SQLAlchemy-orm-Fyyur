...
class Venue(db.Model):
    ...
    shows = db.relationship("Show", lazy=False, cascade="all")
...
def show_venue(venue_id):
  ...
  # }
  venues = Venue.qeury.filter_by(id = venue_id)
  response = list()
  for data in venues:
  ...
  return render_template('pages/show_venue.html', venue=response[0])
...
def create_venue_submission():
  ...
  if form.validate():
      try:
        ...
        db.session.commit()
        flash('Venue ' + request.form['name'] + ' was successful list!')
...
def show_artist(artist_id):
  ...
  response = list()
  for item in data:
    shows = Show.query.filter_by(artist_id = item.id).all()
    past_shows = []
    upcoming_shows = []
    for show in shows:
      if show.start_time >= datetime.date.today():
    ...
    result['upcoming_shows_count'] = len(upcoming_shows)
    result['past_shows_count'] = len(past_shows)
    response.append(result)
  return render_template('pages/show_artist.html', artist=response[0])
